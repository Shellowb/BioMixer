 ...
 */
 Serial.begin(57600);
 pinMode(13, OUTPUT); // for send a binary signal
 pinMode(A0, INPUT);  // for read a analog signal
 pinMode(11, OUTPUT); // send a "analog" signal
 pinMode(10, OUTPUT);
 pinMode(9 , OUTPUT);
}
...
void loop() {
  ...
  //PUT YOUR CODE HERE
  digitalWrite(13,involtPin[13]); // write a binary value in pin 13
  involtSend(0,analogRead(A0));   // read an analog value between 0-1023
  delay(2);
